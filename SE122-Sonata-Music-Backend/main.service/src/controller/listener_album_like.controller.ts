...
export class ListenerAlbumLikeController {
  ...
  /**
   * * GET /album-like/is-liked/:albumId
   */
  async isMyLikedAlbum(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { albumId } = req.params;
      const listener = await getCurrentLoggedUser(req);

      const isLiked = await this.listenerAlbumLikeService.isMyLikedAlbum(Number(albumId), listener.id);

      res.send_ok('Is my liked album fetched successfully', { isLiked: isLiked });
    } catch (error) {
      next(error);
    }
  }

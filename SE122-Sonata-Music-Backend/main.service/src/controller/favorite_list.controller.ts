...
export class FavoriteListController {
  ...
  /**
   * * GET /favorite-list/is-favorite/:musicId
   */
  async isMyFavorite(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { musicId } = req.params;
      const listener = await getCurrentLoggedUser(req);

      const isFavorite = await this.favoriteListService.isMyFavorite(Number(musicId), listener.id);

      res.send_ok('Is my favorite check successfully', { isFavorite });
    } catch (error) {
      next(error);
    }
  }

...
export class AlbumService extends BaseCrudService<Album> implements IAlbumService<Album> {
  ...
  async findOne(options: {
    filter: Partial<Album>;
    relations?: string[];
    select?: any;
  }): Promise<Album | null> {
    const relations = options.relations || [];
    const shouldLoadMusics = relations.includes('musics') || relations.length === 0;
    
    const finalRelations = shouldLoadMusics && !relations.includes('musics') 
      ? [...relations, 'musics'] 
      : relations;
    ...
    if (album.musics && album.musics.length > 0) {
      album.musics = album.musics.filter((music: Music) => music.approved === true);
    }

...
export class FavoriteListService extends BaseCrudService<FavoriteList> implements IFavoriteListService<FavoriteList> {
  ...
  async isMyFavorite(musicId: number, listenerId: number): Promise<boolean> {
    const favorite = await this.favoriteListRepository.findOne({
      filter: {
        musicId: musicId,
  ...
  async removeFromFavoriteList(musicId: number, listenerId: number): Promise<void> {
    await this.favoriteListRepository.findOneAndHardDelete({
      filter: {
        musicId: musicId,
        listenerId: listenerId
      }

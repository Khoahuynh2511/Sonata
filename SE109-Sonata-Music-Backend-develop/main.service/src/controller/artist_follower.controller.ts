...
export class ArtistFollowerController {
  ...
  /**
   * * * GET /follow-artist/is-followed/:artistId
   */
  async isMyFollowedArtist(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { artistId } = req.params;
      const listener = await getCurrentLoggedUser(req);

      const isFollowed = await this.artistFollowerService.isMyFollowedArtist(Number(artistId), listener.id);

      res.send_ok('Is followed artist check successfully', { isFollowed: isFollowed });
    } catch (error) {
      next(error);
    }
  }

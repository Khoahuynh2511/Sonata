...
export class RecommenderController {
  ...
  /**
   * * GET /recommender/timeless-pieces
   */
  async getTimelessPieces(req: Request, res: Response, next: NextFunction) {
    try {
      const listener = await getCurrentLoggedUser(req);
      const topN = parseInt(req.query.topN as string) || 10;

      const timelessPieces = await this.recommenderService.getTimelessPieces(listener.id, topN);

      res.send_ok('Timeless pieces retrieved successfully', timelessPieces);
    } catch (error) {
      next(error);
    }
  }

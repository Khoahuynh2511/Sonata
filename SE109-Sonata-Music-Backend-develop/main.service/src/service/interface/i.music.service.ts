...
export interface IMusicService<T extends BaseModelType> extends IBaseCrudService<T> {
   ...
   */
  createNew(musicCreateReq: MusicCreateReq, contributorId: number): Promise<number>;
   ...
   * @param id Music id
   */
  getById(id: number, userRole: RoleCodeEnum): Promise<MusicDetailRes>;
   ...
   */
  approveById(id: number, adminId: number): Promise<void>;
   ...
   * @param adminId Admin id
   */
  rejectById(id: number, adminId: number): Promise<void>;
  ...
  searchWaitingForApprove(searchData: SearchDataDto): Promise<PagingResponseDto<MusicSearchRes>>;

  /**
   * Search musics
   * @param searchData
   */
  searchMyUpload(searchData: SearchDataDto, contributorId: number): Promise<PagingResponseDto<MusicSearchRes>>;
}

...
export class RecommenderService implements IRecommenderService {
  ...
  constructor(
    ...
    @inject('TimelessPiecesRepository') timeLessPiecesRepository: ITimelessPiecesRepository<TimelessPieces>,
  ...
  async getFromRecommdation(listenerId: number, topN: number, excludeMusicIds: number[]): Promise<Music[]> {
    const recommendations = await redis.get(RedisSchemaEnum.sonata_recommendations + ':' + listenerId);
  ...
  async getTimelessPieces(listenerId: number, topN: number): Promise<Music[]> {
    const timelessPieces = await this.timeLessPiecesRepository.findMany({
      order: [
        {
          column: 'voteCount',
          direction: 'DESC'
    ...
    return timelessPieces.map((timelessPiece) => {
      return timelessPiece.music;
    });
  }

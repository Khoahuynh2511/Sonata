  ...
  implements IListenerAlbumLikeService<ListenerAlbumLike>
{
  ...
  async isMyLikedAlbum(albumId: number, listenerId: number): Promise<boolean> {
    const likedAlbum = await this.listenerAlbumLikeRepository.findOne({
      filter: {
        albumId: albumId,
  ...
  async unLikeAlbum(albumId: number, listenerId: number): Promise<void> {
    await this.listenerAlbumLikeRepository.findOneAndHardDelete({
      filter: {
        albumId: albumId,
        listenerId: listenerId
      }

  ...
  implements IArtistFollowerService<ArtistFollower>
{
  ...
  async isMyFollowedArtist(artistId: number, listenerId: number): Promise<boolean> {
    const artistFollower = await this.artistFollowerRepository.findOne({
      filter: {
        artistId: artistId,
  ...
  async unFollowArtist(artistId: number, listenerId: number): Promise<void> {
    await this.artistFollowerRepository.findOneAndHardDelete({
      filter: {
        artistId: artistId,
        listenerId: listenerId
      }

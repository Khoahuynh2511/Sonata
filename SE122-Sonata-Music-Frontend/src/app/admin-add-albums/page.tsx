...
const AddSongsPage = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!releaseDate || !isValidDate) {
      alert("Vui lòng nhập ngày phát hành hợp lệ (YYYY-MM-DD)");
      return;
    }
    ...
    if (!coverArtUrl) {
      alert("Vui lòng upload ảnh bìa album");
      return;
    }
    ...
    const albumData = {
      ...
      albumType: albumType.trim(),
      description: description.trim() || null,
      genreIds: genreIds.length > 0 ? genreIds : undefined,
      musicIds: musicIds.length > 0 ? musicIds : undefined,
    };
    ...
    try {
      ...
      if (response.data.success) {
        alert(`Album "${albumName}" đã được thêm thành công!`);
        handleClear();
      } else {
        alert(`Lỗi: ${response.data.message || "Không thể thêm album"}`);
      }
    } catch (err: any) {
      console.error("Error adding album:", err);
      const errorMessage = err.response?.data?.message || err.message || "Không thể thêm album";
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <ComboboxMultiSelect
                      ...
                      onSelect={(items) => setSelectedMusics(items as Types.Music[])}
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl shadow-sm border p-6 sticky top-8">
                ...
                <div className="space-y-3">
                  <button
                    onClick={handleAdd}
                    disabled={!albumName.trim() || !releaseDate || !isValidDate || !albumType.trim() || !coverArtUrl}
